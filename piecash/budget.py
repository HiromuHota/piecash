...
class Budget(DeclarativeBaseGuid):
    ...
    def __str__(self):
        return "Budget<{}({}) for {} periods following pattern '{}' >".format(
            self.name, self.description, self.num_periods, self.recurrence
        )
...
class BudgetAmount(DeclarativeBase):
    ...
    budget_guid = Column(
        "budget_guid", VARCHAR(length=32), ForeignKey("budgets.guid"), nullable=False
    )
    account_guid = Column(
